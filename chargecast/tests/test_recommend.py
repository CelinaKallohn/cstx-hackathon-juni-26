...
def _spot_by_hour(frame, lo, hi, lo_before_hour):
    """Per-slot spot: `lo` before the cutoff hour, `hi` from it on."""
    hours = frame['hour'].values
    return np.where(hours < lo_before_hour, lo, hi).astype(float)
...
def test_flatness_penalty_is_scale_invariant():
    ...
    assert flatness_penalty(np.full(SLOTS, 7.0)) == pytest.approx(0.0)


def test_recommended_prices_respect_floor_and_cap(seed_df):
    shape = _fitted_shape(seed_df)
    frame = _day_frame()
    spot = _spot_by_hour(frame, -20.0, 30.0, 6)      # incl. negative spot overnight
    floors = cost_floor_ct(spot, GRID, CONC)
    prices = recommend_prices(shape.predict(frame).copy(), spot,
...
def test_recommendation_flattens_demand_vs_flat_reference(seed_df):
    ...
    rec = recommend_day(fc, frame, np.full(SLOTS, 2.0), CFG, explore=False)
...
def test_total_day_margin_non_negative(seed_df):
    ...
    fc = UnifiedForecaster(shape, PriceEffect(50, 'medium', REF))
    frame = _day_frame()
    spot = _spot_by_hour(frame, -25.0, 20.0, 5)
    ...
    per_slot = forecast_kwh_for_prices(shape.predict(frame), rec['prices'], REF, beta_h)
    slot_margin = per_slot * (rec['prices'] - (spot + GRID + CONC)) / 100.0
    assert np.all(slot_margin >= -1e-9)
...
def test_exploit_is_deterministic(seed_df):
    ...
    fc = UnifiedForecaster(shape, PriceEffect(50, 'medium', REF))
    frame = _day_frame()
    a = recommend_day(fc, frame, np.full(SLOTS, 2.0), CFG, explore=False)
    b = recommend_day(fc, frame, np.full(SLOTS, 2.0), CFG, explore=False)
...
def test_exploration_is_per_block(seed_df):
    ...
    hours = frame['hour'].values
    midday_mask = np.isin(hours, pe.blocks['midday'])
    tight_mask = np.isin(hours, pe.blocks['overnight'])
    # the wide block explores far more; tight-block residual is just grid-step jitter
    assert spread[midday_mask].mean() > 3 * spread[tight_mask].mean()
