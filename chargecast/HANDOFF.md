...
1. **DemandShapeModel** (`core.py`) — price-neutral demand. v0.1 profile/GBM
...
2. **BlockBeta** (`core.py`) — the conjugate Normal–Normal engine for ONE block's
   coefficient: `update(price_dev, y)`, reporting (`beta`/`beta_ci`/`pct`/`pct_ci`),
   `sample_beta` (Thompson). No-signal → posterior stays at prior.
   **PriceEffect** (`core.py`) — holds one BlockBeta per block (`coeffs` dict),
...
3. **UnifiedForecaster** (`core.py`) — combines shape + grouped price effect;
...
4. **cost_floor_ct** (`core.py`) — break-even `spot+grid+concession`; negative
